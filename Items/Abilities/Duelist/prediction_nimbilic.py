...
class prediction_nimbilic:
    ...
    name = f"{colors.LightMagenta}Prediction: Nimbilic{colors.Reset}"
    desc = f"A good duelist studies their opponent. When they are confident, they predict your next move. And if they're right, they suddenly become confident enough to strike even harder.\n" \
           f"Predict your opponent's third swing style. If it is a Brawler ability, gain an extra swing on your next turn. If you've guessed wrong, take damage equal to 15% of your maxHP. This goes on cooldown for 4 turns." \
    ...
    def special(self, combat, caster, casted):
        ...
        if self.cur_cd <= 0 and combat.swings[0].name == prediction_nimbilic.name and combat.swing != 0 and combat.mobs[0] == caster:
    ...
    def check_effect(self, combat, caster, casted):
        ...
        if effect_count < prediction.stack:  # changes how many times this effect can stack
            combat.effects[casted.type].append(prediction(combat, caster, casted, nimbilic, 1))
