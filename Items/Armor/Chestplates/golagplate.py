...
class golagplate:
    ...
    # MATERIAL: AMOUNT
    recipe = {golagtatite: 5, wolf_pelt: 3}
    ...
    name = f"{colors.LightGreen}Golagplate{colors.Reset}"

    # ADD ALL VARIABLES CHANGED TO DESCRIPTION
    desc = f"A plate of Golagtatite, and some Wolf Pelts for style. This beefy chest piece is sure to slow you down." \
           ...
           f"\n{colors.Yellow}Armor%: {colors.LightYellow}{armor_percent * 100}%" \
    ...
    def print_recipe():
        string = ""
        for key in golagplate.recipe:
            string = f"{string}| {golagplate.recipe[key]}x {key.name} | "
