...
class rustic_boots:
    ...
    name = f"{colors.White}Rustic Boots{colors.Reset}"

    # ADD ALL VARIABLES CHANGED TO DESCRIPTION
    desc = f"These boots have had better days, but it still protects you, technically..." \
    ...
    def print_recipe():
        string = ""
        for key in rustic_boots.recipe:
            string = f"{string}| {rustic_boots.recipe[key]}x {key.name} | "
